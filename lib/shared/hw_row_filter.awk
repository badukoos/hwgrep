# Shared row filtering for table renderers.
#
# Bash passes:
#   -v row_filter="key=val\nkey2=val2\n..."
#
# Semantics:
# - Multiple clauses are ANDed.
# - Within a clause, comma/pipe are OR (val1,val2 or val1|val2).
# - Case-insensitive.
# - Matching:
#     status: exact match on normalized status (works/detected/failed/...)
#     others: substring match (contains)
...
function row_filter_init(   s, n, i, clause, eq, k, v, key, m, j, t) {
  ...
  # Split clauses by newline or semicolon
  n = split(s, rf_clause, /[\n;]/)
  for (i = 1; i <= n; i++) {
...
function row_filter_value_match(key, val,   v, i, tok) {
  ...
  # If key not requested, it doesn't constrain matching
